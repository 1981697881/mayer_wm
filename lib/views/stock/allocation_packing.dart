...
class AllocationPacking extends StatefulWidget {
  var FBillNo;
  var tranType;
  ...
  @override
  _AllocationPackingState createState() =>
      _AllocationPackingState(FBillNo, tranType);
}

class _AllocationPackingState
    extends State<AllocationPacking> {
  ...
  void _onEvent(event) async {
    ...
    if (this._positionContent.text == '') {
      ...
      if (jsonDecode(order)['success'] && jsonDecode(order)['data'].length>0) {
    ...
    if (materialDate != null) {
      ...
      if (number == 0) {
        ...
        arr.add({
          ...
          "isHide": false,
          "value": {
            "label": materialDate['remainQty'].toString(),
            "value": materialDate['remainQty'].toString()
          }
        });
        arr.add({
          "title": "已打标签",
  ...
  saveOrder() async {
    ...
    if (this.hobby.length > 0) {
      ...
      this.hobby.forEach((element) {
        ...
        FEntityItem['packNo'] = element[0]['value']['value'];
        FEntityItem['barcode'] = element[1]['value']['value'];
        FEntityItem['qty'] = element[2]['value']['value'];
        FEntityItem['date'] = FDate;
        FEntityItem['orderBillNo'] = fBillNo;
        FEntityItem['printCount'] = 1;
        FEntityItem['tranType'] = tranType;
        FEntity.add(FEntityItem);
        hobbyIndex++;
      });
      var saveData = jsonEncode(FEntity);
      ToastUtil.showInfo('保存');
      String order = await SubmitEntity.packBox(FEntity);
                        ...
                        child: ListTile(
                          title: TextField(
                            decoration: InputDecoration(
                                labelText: "箱号", border: OutlineInputBorder()),
                            controller: this._positionContent,
                            //改变回调
                            onChanged: (value) {
                              setState(() {
                                _positionContent.value = TextEditingValue(
                                    text: value,
