...
class AllocationTallying extends StatefulWidget {
  var FBillNo;
  var tranType;

  AllocationTallying({Key? key, @required this.FBillNo, @required this.tranType})
      : super(key: key);

  @override
  _AllocationTallyingState createState() =>
      _AllocationTallyingState(FBillNo, tranType);
}

class _AllocationTallyingState extends State<AllocationTallying> {
  ...
  getMaterialList(barcodeData, code, str) async {
    ...
    if(code.split('-').length<=1){
      ToastUtil.showInfo("当前扫描条码非装箱码");
      return;
    }
    //userMap['barcode'] = code;
    //userMap['orderBillNo'] = code.split('-')[0];
    userMap['packNo'] = code;
    ...
    if (materialDate.length>0) {
      var msg = "";
      for (var meterialItem in materialDate) {
        var number = 0;
        var barCodeScan = meterialItem;
        // 检查是否为整数
        if (meterialItem['qty'] % 1 == 0) {
          meterialItem['qty'] = meterialItem['qty'].toInt();
        }
        var barcodeNum = meterialItem['qty'];
        var barcodeQuantity = meterialItem['qty'];
        var fsn = barcodeNum;
        var orderIndex = 0;
        for (var value in orderDate) {
          if (value['FItemNumber'] == meterialItem['itemNumber']) {
            msg = "";
            if (fNumber.lastIndexOf(meterialItem['itemNumber']) == orderIndex) {
              break;
            }
          } else {
            msg = '条码不在单据物料中';
        ...
        for (var element in hobby) {
          ...
          if (element[5]['isHide']) {
            //不启用
            if (element[0]['value']['value'] == barCodeScan['itemNumber']) {
                    ...
                    if ((int.parse(element[3]['value']['label']) +
                      ...
                      if (element[0]['value']['scanCode'].indexOf(code) == -1) {
                        ...
                        element[10]['value']['remainder'] = (
                            int.parse(element[10]['value']['value']) -
                                int.parse(barcodeNum))
                            .toString();
              ...
              if (element[0]['value']['barcode'].indexOf(code) == -1) {
                ...
                if (element[5]['value']['value'] == barCodeScan['batchNo']) {
                  ...
                  if (element[5]['value']['value'] == "") {
                    ...
                    barCodeScan['batchNo'] == null ? "" : barCodeScan['batchNo'];
        ...
        if (number == 0 && this.fBillNo == "") {
          ...
          arr.add({
            ...
            "isHide": false,
            "value": {
              "label": meterialItem["name"] + "- (" + meterialItem["number"] + ")",
              "value": meterialItem["number"],
              "barcode": [code],
          ...
          arr.add({
            ...
            "isHide": false,
            "name": "FMaterialIdFSpecification",
            "value": {"label": meterialItem["model"], "value": meterialItem["model"]}
          });
          arr.add({
            "title": "单位名称",
            "name": "FUnitId",
            "isHide": false,
            "value": {"label": meterialItem["unitName"], "value": meterialItem["unitNumber"]}
          });
          arr.add({
            "title": "实收数量",
            "name": "FRealQty",
            "isHide": false,
            /*value[12]*/
            "value": {"label": meterialItem["quantity"].toString(), "value": meterialItem["quantity"].toString()}
          });
          arr.add({
            "title": "仓库",
            "name": "FStockID",
          ...
          arr.add({
            "title": "批号",
            "name": "FLot",
            "isHide": !meterialItem["batchManager"],
            "value": {
              "label": meterialItem["batchNo"],
              "value": meterialItem["batchNo"]
            }
          });
          arr.add({
            "title": "库位",
          ...
          arr.add({
            ...
            "name": "FLastQty",
            "isHide": false,
            "value": {"label": meterialItem["quantity"].toString(), "value": meterialItem["quantity"].toString(),"remainder": "0","representativeQuantity": meterialItem["quantity"].toString()}
          });
          hobby.add(arr);
        }
        setState(() {
  ...
  saveOrder() async {
    ...
    if (this.hobby.length > 0) {
      ...
      this.hobby.forEach((element) {
        if (element[3]['value']['value'] != '0') {
          ...
          for (int subj = 0; subj < kingDeeCode.length; subj++) {
            ...
            FEntityItem['date'] = FDate;
            FEntityItem['barcode'] = itemCode[0]+'-'+itemCode[1];
            FEntityItem['orderBillNo'] = this.fBillNo;
            FEntityItem['qty'] = itemCode[2];
            FEntityItem['seq'] = hobbyIndex+1;
            FEntityItem['tranType'] = 41;
      ...
      String order = await SubmitEntity.preStockOut(FEntity);
