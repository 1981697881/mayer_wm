...
import 'package:mayer_wm/model/version_entity.dart';
import 'package:mayer_wm/utils/SqfLiteQueueDataOffline.dart';
import 'package:mayer_wm/utils/SqfLiteQueueDataRepertoire.dart';
import 'package:mayer_wm/utils/SqfLiteQueueDataScheme.dart';
import 'package:mayer_wm/utils/icons.dart';
import 'package:mayer_wm/utils/menu_permissions.dart';
...
class _IndexPageState extends State<IndexPage> {
  ...
  var iconMap = MenuIcon.getIcon();
  IconData getIconDataFromString(String iconName) {
    // 使用反射获取IconData
    IconData iconData;
    if (iconMap.containsKey(iconName)) {
      iconData = iconMap[iconName]!;
    } else {
      // 如果未找到，则返回默认图标或抛出异常
      iconData = Icons.help;
    }
...
/*一个渐变颜色的正方形集合*/
  List<Widget> Boxs(List<Map<String, dynamic>> menu) =>
      List.generate(menu.length, (index) {
        return InkWell(
            onTap: () {
              if(menu[index]['source'] == null){
                Navigator.pushNamed(context, menu[index]['router']);
              }else{
                Navigator.pushNamed(context, menu[index]['router'],
                    arguments: menu[index]['source']);
              }
              /*Navigator.push(
                context,
                MaterialPageRoute(
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(
                        getIconDataFromString(menu[index]['icon']),
  ...
  Widget buildAppBarTabs() {
    ...
    var childMenu = MenuPermissions.getMenuChild(deptData);
            ...
            body: new ListView(padding: EdgeInsets.all(10), children: <Widget>[
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  /*prefs.clear();*/
                  if (await SqfLiteQueueDataOffline.isTableExits(
                          "offline_Inventory") ==
                      true) {
                  ...
                  if (await SqfLiteQueueDataRepertoire.isTableExits(
                          "barcode_list") ==
                      true) {
                    SqfLiteQueueDataRepertoire.deleteDataTable();
                  }
                  if (await SqfLiteQueueDataScheme.isTableExits(
                          "scheme_Inventory") ==
                      true) {
