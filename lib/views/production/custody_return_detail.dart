...
class CustodyReturnDetail extends StatefulWidget {
  var FBillNo;
  var tranType;
  ...
  @override
  _CustodyReturnDetailState createState() => _CustodyReturnDetailState(
      FBillNo, tranType);
}

class _CustodyReturnDetailState extends State<CustodyReturnDetail> {
  ...
  getBillNo() async {
    String res = await CurrencyEntity.getBillNo("102");
    if (jsonDecode(res)['success']) {
      setState(() {
    ...
    if (materialDate != null) {
      ...
      if (number == 0 && this.fBillNo == "") {
        ...
        arr.add({
          ...
          "name": "FPrdOrgId",
          "isHide": true,
          "value": {"label": materialDate["remainQty"], "value": materialDate["remainQty"], "rateValue": materialDate["remainQty"]}
        });
        arr.add({
          "title": "仓库",
          "name": "FStockId",
  ...
  saveOrder() async {
    ...
    if (this.hobby.length > 0) {
      ...
      this.hobby.forEach((element) {
        if (element[9]['value']['value'] != '0' && element[9]['value']['value'] != '') {
          ...
          for (int subj = 0; subj < kingDeeCode.length; subj++) {
            ...
            Map<String, dynamic> FrameEntityItem = Map();
            FrameEntityItem['uuid'] = itemCode[0];
      ...
      print(res);
      if (res['success']) {
        String orderFrame = await SubmitEntity.onFrame(FrameEntity);
        var resFrame = jsonDecode(orderFrame);
        if (resFrame['success']) {
          //提交清空页面
          setState(() {
            this.hobby = [];
