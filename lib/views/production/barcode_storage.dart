...
class BarcodeStorage extends StatefulWidget {
  var FBillNo;
  var tranType;

  BarcodeStorage({Key? key, @required this.FBillNo, @required this.tranType})
      : super(key: key);

  @override
  _BarcodeStorageState createState() =>
      _BarcodeStorageState(FBillNo, tranType);
}

class _BarcodeStorageState extends State<BarcodeStorage> {
      ...
      this.hobby.forEach((element) {
        if (element[3]['value']['value'] != '0') {
          ...
          for (int subj = 0; subj < kingDeeCode.length; subj++) {
            ...
            subObj['billType'] = "条码入库";
            subObj['barcodeListId'] = itemCode[3];
            subObj['inQty'] = element[3]['value']['value'];
