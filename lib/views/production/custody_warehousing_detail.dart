...
class CustodyWarehousingDetail extends StatefulWidget {
  var FBillNo;
  var tranType;

  CustodyWarehousingDetail({Key? key, @required this.FBillNo, @required this.tranType})
      : super(key: key);

  @override
  _CustodyWarehousingDetailState createState() =>
      _CustodyWarehousingDetailState(FBillNo, tranType);
}

class _CustodyWarehousingDetailState extends State<CustodyWarehousingDetail> {
  ...
  getTypeList() async {
    ...
    if (jsonDecode(res)['success']) {
      ...
      if(typeList.length == 1){
        this.typeName = typeListObj[0]['FBillTypeName'];
        this.typeNumber = typeListObj[0]['FBillType'];
      }
    }else{
  ...
  getBillNo() async {
    String res = await CurrencyEntity.getBillNo("101");
    if (jsonDecode(res)['success']) {
      setState(() {
    ...
    if (orderDate.length > 0) {
      ...
      orderDate.forEach((value) {
        ...
        arr.add({
          ...
          "name": "FStockID",
          "isHide": false,
          "value": {"label": value['FDefaultStockName'], "value": value['FDefaultStockNumber']}
        });
        arr.add({
          "title": "批号",
          "name": "FLot",
          ...
          if (element[0]['value']['value'] == barCodeScan['number']) {
            if (element[0]['value']['barcode'].indexOf(code) == -1) {
              ...
              if (element[4]['value']['value'] == "") {
                element[4]['value']['label'] = materialDate["FDefaultStockName"] == null? "":materialDate["FDefaultStockName"];
                element[4]['value']['value'] = materialDate["FDefaultStockNumber"] == null? "":materialDate["FDefaultStockNumber"];
              }
              //判断扫描数量是否大于单据数量
      ...
      onConfirm: (p) {
        ...
        setState(() {
          if (hobby == 'supplier') {
            ...
            data.forEach((element) {
              if (element == p) {
                stockNumber = stockListObj[elementIndex]['FNumber'];
              }
              elementIndex++;
            });
          } else {
              ...
              listTitle.add(
                ListTile(
                  ...
                  trailing:IconButton(
                    ...
                    onPressed: () {
                      this.hobby[i][0]["value"]['kingDeeCode'].remove(dataItem);
                      setState(() {});
                    },
                  ),
                ),
