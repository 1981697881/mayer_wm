...
class PipeRemovalDetail extends StatefulWidget {
  var FBillNo;
  var tranType;

  PipeRemovalDetail({Key? key, @required this.FBillNo, @required this.tranType})
      : super(key: key);

  @override
  _PipeRemovalDetailState createState() =>
      _PipeRemovalDetailState(FBillNo, tranType);
}

class _PipeRemovalDetailState extends State<PipeRemovalDetail> {
                  ...
                  if ((double.parse(element[9]['value']['label']) +
                    ...
                    if (element[0]['value']['scanCode'].indexOf(code) == -1) {
                      ...
                      ['label'] -
                          double.parse(element[9]['value']['label']))
                          .toString();
                      if(double.parse(element[10]['value']['value']) >= double.parse(barcodeNum)){
                        element[9]['value']['remainder'] = (
                            double.parse(element[10]['value']['value']) - double.parse(barcodeNum))
                            .toString();
                      }else{
                        element[9]['value']['remainder'] = (
                            double.parse(barcodeNum) - double.parse(element[10]['value']['value']))
                            .toString();
                      }
                      element[9]['value']['representativeQuantity'] = barcodeQuantity;
                    ...
                    if (element[0]['value']['scanCode'].indexOf(code) == -1) {
                      ...
                      element[9]['value']['remainder'] = residue.toString();
                      element[9]['value']['representativeQuantity'] = barcodeQuantity;
                      element[0]['value']['kingDeeCode'].add(item);
                      if(barCodeScan['isEnable'] == 1){
                        element[0]['value']['scanCode'].add(code);
                           ...
                           if (checkItem == "FLastQty") {
                              if(double.parse(_FNumber) <= double.parse(this.hobby[checkData][checkDataChild]["value"]['representativeQuantity'])){
                                if (this.hobby[checkData][0]['value']['kingDeeCode'].length > 0) {
                                  ...
                                  this.hobby[checkData][9]["value"]["remainder"] = (Decimal.parse(this.hobby[checkData][9]["value"]["representativeQuantity"]) - Decimal.parse(_FNumber)).toString();
